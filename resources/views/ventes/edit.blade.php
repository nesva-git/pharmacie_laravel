        ...
        <div class="col-lg-12">
            <div class="card shadow mb-4">
                <div class="card-header py-3 d-flex justify-content-between align-items-center">
                    <h6 class="m-0 font-weight-bold text-primary">Modifier la Vente #{{ $vente->id }}</h6>
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('ventes.update', $vente->id) }}" method="POST">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('produit_id') is-invalid @enderror" id="produit_id" name="produit_id" required>
                                        ...
                                        @foreach($produits as $produit)
                                            <option value="{{ $produit->id }}" data-prix="{{ $produit->prix }}" data-stock="{{ $produit->quantite_stock + ($produit->id == $vente->produit_id ? $vente->quantite : 0) }}" {{ (old('produit_id', $vente->produit_id) == $produit->id) ? 'selected' : '' }}>
                                                {{ $produit->nom }} - {{ number_format($produit->prix, 2) }} FCFA (Stock: {{ $produit->quantite_stock + ($produit->id == $vente->produit_id ? $vente->quantite : 0) }})
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('produit_id')
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('pharmacien_id') is-invalid @enderror" id="pharmacien_id" name="pharmacien_id">
                                        ...
                                        @foreach($pharmaciens as $pharmacien)
                                            <option value="{{ $pharmacien->id }}" {{ (old('pharmacien_id', $vente->pharmacien_id) == $pharmacien->id) ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" min="1" class="form-control @error('quantite') is-invalid @enderror" id="quantite" name="quantite" value="{{ old('quantite', $vente->quantite) }}" required>
                                        @error('quantite')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="datetime-local" class="form-control @error('date_vente') is-invalid @enderror" id="date_vente" name="date_vente" value="{{ old('date_vente', \Carbon\Carbon::parse($vente->date_vente)->format('Y-m-d\TH:i')) }}" required>
                                        @error('date_vente')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" step="0.01" min="0" class="form-control @error('total') is-invalid @enderror" id="total" name="total" value="{{ old('total', $vente->total) }}" readonly>
                                        @error('total')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="alert alert-info" id="recap-vente">
                        ...
                        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                            <a href="{{ route('ventes.index') }}" class="btn btn-secondary me-md-2">
